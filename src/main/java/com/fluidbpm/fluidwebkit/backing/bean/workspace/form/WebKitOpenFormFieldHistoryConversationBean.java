...
public class WebKitOpenFormFieldHistoryConversationBean extends ABaseManagedBean {
    ...
    public static class FlatFieldHistory implements Serializable {
        ...
        public String getDisplayValueForTimeDiff() {
            ...
            if (takenInDays > 0)
                returnVal.append(String.format("%s day%s, ", takenInDays, takenInDays == 1 ? UtilGlobal.EMPTY : "s"));
            if (takenInHours > 0)
                returnVal.append(String.format("%s hour%s, ", takenInHours, takenInHours == 1 ? UtilGlobal.EMPTY : "s"));
            if (takenInMinutes > 0)
                returnVal.append(String.format("%s minute%s, ", takenInMinutes, takenInMinutes == 1 ? UtilGlobal.EMPTY : "s"));
            if (takenInSeconds > 0)
                returnVal.append(String.format("%s second%s, ", takenInSeconds, takenInSeconds == 1 ? UtilGlobal.EMPTY : "s"));
        ...
        public boolean isValueOfTypeSignature() {
            ...
            switch (this.field.getTypeAsEnum()) {
                ...
                case ParagraphText:
                    ...
                    if (UtilGlobal.isBlank(val)) return false;
                    JsonObject sigJSON = JsonParser.parseString(val).getAsJsonObject();
