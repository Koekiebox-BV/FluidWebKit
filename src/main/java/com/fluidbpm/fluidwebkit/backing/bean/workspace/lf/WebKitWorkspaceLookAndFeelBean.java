...
public class WebKitWorkspaceLookAndFeelBean extends ABaseManagedBean {
    ...
    private void populatePersonalInventory(ConfigurationListing configurationListing) {
        ...
        this.webKitPersonalInventory = new WebKitPersonalInventory(
                JsonParser.parseString(jsonVal).getAsJsonObject()
        );
    }
    ...
    private void populateUserQueryMenu(ConfigurationListing configurationListing) {
        ...
        this.webKitGlobal = new WebKitGlobal(
                JsonParser.parseString(jsonVal).getAsJsonObject()
        );
    ...
    public List<SelectItem> extractFieldsForUserToFormFieldMCFrom(String formDef) {
        if (this.userToFormFieldLimitOnMultiChoiceByFormDef == null)
            this.userToFormFieldLimitOnMultiChoiceByFormDef = new HashMap<>();

        if (this.userToFormFieldLimitOnMultiChoiceByFormDef.containsKey(formDef))
            return this.userToFormFieldLimitOnMultiChoiceByFormDef.get(formDef);
